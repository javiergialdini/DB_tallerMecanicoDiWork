...
/*+ Se generan los registros en la tabla temporal que posteriormente se evaluarán para ver si procede su carga en la tabla definitiva de Repuestos*/
	...
	-- CREO TABLA TEMPORAL PARA ACUMULAR EL PRECIO DE LOS RESPUESTOS IGUALES
	CREATE TABLE #TMP_RESPUESTO_ACUM (Nombre VARCHAR(100),
                                 Precio DECIMAL(18,6))
	
	insert into #TMP_RESPUESTO_ACUM (Nombre, Precio)
		SELECT
			tmp.Nombre,
			SUM(tmp.Precio)
		FROM
	...
	-- BORRO REGISTROS DE TABLA REPUESTOS SI LOS HAY
	BEGIN
		DELETE FROM [dbo].[Repuesto]
	END
	BEGIN

	-- INSERTO EN TABLA Repuesto LOS REGISTROS CON PRECIO MENOR A 20
	insert into [dbo].[Repuesto] (Nombre, Precio)
		SELECT
			tmp.Nombre,
			tmp.Precio
		FROM
			#TMP_RESPUESTO_ACUM tmp
		WHERE
			Precio < 20
	END

	-- RETORNO LOS REPUESTOS CON PRECIO MAYOR O IGUAL A 20
	BEGIN
		SELECT
			tmp.Nombre,
			tmp.Precio
